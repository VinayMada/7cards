...
function WaitingRoom({ roomId, myName, isHost, onGameStart }) {
  ...
  const startGame = async () => {
    ...
    const gs = {
      ...
      sevenPenalty: 0,
      lastDropRank: null,   // ← tracks previous drop rank for clash detection
...
function GameScreen({ roomId, myName, initialState }) {
  ...
  useEffect(() => {
    const unsub = roomListen(roomId, r => { setGs(r); setSelected([]); });
    return unsub;
  }, [roomId]);
  ...
  const doAction = async () => {
    ...
    // ── BUG FIX: 7-penalty — validate counter before allowing drop ──
    if ((r.sevenPenalty || 0) > 0) {
      if (!isValidSevenCounter(dropping)) {
        // Player tried to drop non-counter cards while penalty is active — block it
        flash(`Must counter with 7s or 3+ same-rank cards, or click "Draw ${r.sevenPenalty * 2}"`);
        return; // Do NOT proceed with drop
      }
      // Valid counter: if using 3+ same-rank (tackle), clear penalty
      if (dropRank !== "7") {
        r.sevenPenalty = 0;
      }
      // If dropping 7s, the penalty chain logic below handles adding to sevenPenalty
    }
    ...
    // ── CLASH: if dropped rank matches lastDropRank, no draw needed ──
    // lastDropRank is set from the previous player's drop.
    // Clash applies to any rank EXCEPT 7 (7 has its own chain rule) and J (J never draws anyway)
    const isClash = !!(r.lastDropRank && dropRank === r.lastDropRank && dropRank !== "7" && dropRank !== "J");
    if (isClash) {
      logMsg += ` ⚡ CLASH! No draw needed`;
    }

    // ── J rule: skip N players where N = number of Js dropped. Dropper never draws ──
    if (dropRank === "J") {
      skipCount = dropping.length;
      logMsg += ` → ${skipCount} player${skipCount > 1 ? "s" : ""} SKIPPED!`;
    }

    // ── 7 rule: chain penalty forward ──
    if (dropRank === "7") {
    ...
    // Draw 1 card UNLESS:
    //   - dropped > 2 cards
    //   - dropped J (J never draws)
    //   - dropped 7 (penalty card, no draw)
    //   - CLASH (same rank as previous drop → no draw)
    const mustDraw = dropping.length <= 2
      && dropRank !== "J"
      && dropRank !== "7"
      && !isClash;

    if (mustDraw && r.drawPile.length > 0) {
    ...
    // ── Store lastDropRank for next player's clash check ──
  ...
  const dropLabel = (() => {
    ...
    return `DROP ${selected.length} ${selected.length > 1 ? "CARDS" : "CARD"}${willDraw ? " + DRAW" : ""}${isClashDrop ? " ⚡ CLASH" : ""}`;
  })();

  // Instruction hint
  const hintText = (() => {
    if (!isMyTurn) return `Waiting for ${currentPlayerName}…`;
    if (penaltyActive) {
    ...
    if (!selected.length) return "Your turn! Tap same-rank cards to select, then DROP.";
    ...
    <div className="game-wrap">
      ...
      <div className="table-center">
        <div className="felt-surface">
          ...
          {/* Discard pile — glow orange when clash is possible */}
          <div className="felt-group">
            <div className="felt-label">
              DISCARD {isMyTurn && lastDropRank && !penaltyActive ? <span className="clash-badge">⚡ CLASH</span> : ""}
            </div>
            {gs.discardPile?.length > 0 ? (
              <div className={`card card-sm ${isRed(gs.discardPile[0].suit) ? "red" : "blk"} ${isMyTurn && lastDropRank && !penaltyActive ? "clash-glow" : ""}`}>
