...
function GameScreen({ roomId, myName, initialState }) {
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      ...
      setTimeLeft(left);
      // Auto-advance when timer hits 0 and it's this client's turn
  ...
  const doAction = async () => {
    ...
    // If hand is now empty, must assign one card from draw pile
    if (hand.length === 0) {
      reshuffleIfNeeded(r);
  ...
  const autoAdvanceTurn = async () => {
    ...
    const aPlayers = r.players.filter(p => !p.eliminated);
    const curIdx = (r.currentPlayer || 0) % aPlayers.length;
    if (aPlayers[curIdx]?.name !== myName) return; // only current player triggers
    const hand = [...(r.hands[myName] || [])];
    if (hand.length === 0) return;
    // Drop the highest value card
    ...
    // If hand is empty after drop, assign one from draw
    if (hand.length === 0) {
      reshuffleIfNeeded(r);
      if (r.drawPile.length > 0) hand.push(r.drawPile.shift());
    } else {
      // Draw from pile automatically
      reshuffleIfNeeded(r);
      if (r.drawPile.length > 0) hand.push(r.drawPile.shift());
    }
    r.hands[myName] = hand;
    ...
    const now = Date.now();
    r.currentPlayer = computeNext(curIdx, 0, aPlayers.length);
    r.currentPlayerName = aPlayers[computeNext(curIdx, 0, aPlayers.length)]?.name;
    r.turnStartedAt = now;
    r.log = [`⏱️ ${myName} timed out — auto-dropped ${dropped.rank}${dropped.suit}`, ...(r.log || []).slice(0, 14)];
    await roomSet(roomId, r);
  };
  ...
  const nextRound = async () => {
    ...
    const nextState = {
      ...
      currentPlayer: 0, currentPlayerName: active[0]?.name,
      turnStartedAt: Date.now(),
      sevenPenalty: 0, lastDropRank: null,
          ...
          <>
            <div className="my-header">
              ...
              <div className="my-actions">
                ...
                {/* Show button — only on your turn AND last drop was not 7 */}
                {isMyTurn && myCount <= 5 && !gs.roundOver && lastDropRank !== "7" && !penaltyActive && (
...
.kick-btn{margin-left:auto;background:rgba(231,76,60,0.15);border:1px solid rgba(231,76,60,0.3);color:#e74c3c;border-radius:6px;width:22px;height:22px;font-size:11px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.15s;}
.kick-btn:hover{background:rgba(231,76,60,0.35);}
.tb-timer{min-width:60px;text-align:right;}
.timer-ring{font-size:13px;font-weight:700;color:rgba(240,235,224,0.5);transition:color 0.3s;}
.timer-warn{color:#e67e22;}
.timer-urgent{color:#e74c3c;animation:blink-timer 0.5s infinite;}
